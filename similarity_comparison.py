...
# Assuming a function `plot_growth` exists that takes the age-height pairs, 
# finds similar growth patterns, calculates median, std deviation, and plots the figure
def plot_growth(age_height_pairs):
    ...
    #std_deviation = similar_growth_curves.std()

    # Alternatively, you can use the interquartile range (IQR)
    iqr = np.subtract(*np.percentile(similar_growth_curves, [75, 25], axis=0))
    predicted_height_at_18 = median_heights.loc[18]

    # Plot the results
    plt.figure(figsize=(10, 5))
    plt.title('Predicted Growth Curve Based on Similar Patterns')
    plt.xlabel('Age')
    plt.ylabel('Height (cm)')

    # Plot the median projected growth curve
    plt.plot(ages, median_heights, label='Median Projected Growth', color='blue', marker='o')

    # Plot the average cloud around the median using the standard deviation
    #plt.fill_between(ages, (median_heights - std_deviation), (median_heights + std_deviation), color='skyblue', alpha=0.5, label='Average Cloud')

    # If using IQR, uncomment the following line
    plt.fill_between(ages, (median_heights - iqr), (median_heights + iqr), color='skyblue', alpha=0.5, label='IQR')

    # Plot input data points
    input_ages, input_heights = zip(*age_height_pairs)
    plt.scatter(input_ages, input_heights, color='red', label='Input Data Points', zorder=5)
    plt.scatter(18, predicted_height_at_18, color='green', label=f'Predicted Height at 18', zorder=5, s=100)
